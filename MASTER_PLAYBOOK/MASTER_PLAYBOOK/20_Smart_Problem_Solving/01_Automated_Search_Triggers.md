...
interface PatternBasedTrigger {
  ...
  repetitiveAttempts: {
    ...
    examples: ['같은 명령어 반복 실행', '동일한 코드 수정 반복', '비슷한 구글링 키워드 반복']
  }
...
interface TechnologySpecificTriggers {
  ...
  frontend: {
    ...
    nextjs: {
      quickTriggers: ['Hydration 에러', 'Routing 문제', 'Build 에러', 'API Routes 에러']
      searchPatterns: [
    ...
    vue: {
      quickTriggers: ['Composition API 에러', 'Reactivity 문제', 'Template 컴파일 에러']
      searchPatterns: [
  ...
  backend: {
    ...
    python: {
      quickTriggers: ['Import 에러', 'Virtual environment 문제', 'Package 의존성 충돌']
      searchPatterns: [
  ...
  devops: {
    docker: {
      quickTriggers: ['Container build 실패', 'Network 연결 문제', 'Volume mount 에러']
      searchPatterns: [
    ...
    kubernetes: {
      quickTriggers: ['Pod 상태 에러', 'Service 연결 실패', 'Resource 할당 문제']
      searchPatterns: [
...
class ContextAwareTriggerSystem {
  ...
  private generateSearchQuery(technology: TechnologyStack, trigger: QuickTrigger): string {
