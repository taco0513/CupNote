...
class PatternGeneralizer {
  ...
  private async buildSuccessPredictionModel(
    ...
    // 특성 벡터 생성
    const featureVectors = patternGroup.map(pattern => this.extractFeatureVector(pattern))
...
class IntelligentPatternMatcher {
  async findBestMatchingPatterns(
    ...
    // 종합 점수 계산 및 순위 매기기
    const rankedMatches = this.rankPatternMatches(contextuallyRelevant, successProbabilities)
    ...
    return {
      ...
      adaptationRequirements: this.identifyAdaptationRequirements(rankedMatches[0], problemContext),
    }
  }
  ...
  private async predictSuccessProbability(
    ...
    const adjustedProbability =
      modelPrediction * similarityAdjustment * contextualAdjustment * performanceAdjustment
