...
class AutoDocumentationEngine {
  ...
  async generateDocumentation(sources: DocumentationSource[]): Promise<DocumentationResult> {
    // 1. 소스 분석 및 컨텍스트 추출
    const analysisResults = await Promise.all(sources.map(source => this.analyzeSource(source)))

    // 2. 문서 구조 설계
    const documentStructure = await this.designDocumentStructure(analysisResults)

    // 3. 콘텐츠 생성
    const generatedContent = await this.generateContent(documentStructure, analysisResults)
  ...
  async generateClassDocumentation(
    ...
    return {
      ...
      properties: await Promise.all(
        classDef.properties.map(prop => this.generatePropertyDocumentation(prop, context))
      ),

      methods: await Promise.all(
        classDef.methods.map(method => this.generateFunctionDocumentation(method, context))
      ),
...
class APIDocumentationGenerator {
  async generateAPIDocumentation(
    ...
    const endpoints = await Promise.all(
      apiDefinition.endpoints.map(endpoint => this.documentEndpoint(endpoint, codebase))
    )
...
class DocumentationVersionControl {
  ...
  async trackDocumentChanges(
    ...
    if (updateNeeded) {
      ...
      // 버전 정보 기록
      const versionInfo = await this.createVersionRecord(document, updatedDocument, sourceChanges)
  ...
  async validateDocumentConsistency(
    ...
    for (const doc of documents) {
      const codeInconsistencies = await this.checkCodeDocumentConsistency(doc, codebase)
...
{{/each}}
  ...
  async renderTemplate(
    ...
    const renderedSections = await Promise.all(
      finalTemplate.sections.map(section => this.renderSection(section, variables))
    )
...
class MultilingualDocumentationSystem {
  ...
  async generateMultilingualDocumentation(
    ...
    const translations = await Promise.all(
      targetLanguages.map(async lang => {
        ...
        // 문화적 적응
        const culturallyAdapted = await this.applyCulturalAdaptations(translated, lang)

        // 기술 용어 현지화
        const localized = await this.applyTechnicalLocalization(culturallyAdapted, lang)
  ...
  async translateTechnicalDocument(
    ...
    // 단계별 번역
    const translatedContent = await this.translateWithContext(document.content, targetLanguage, {
      codeBlocks,
      technicalTerms,
...
class LiveDocumentationSystem {
  ...
  // 파일 변경 처리
  private async handleFileChange(filePath: string, config: LiveDocConfig): Promise<void> {
    // 변경된 파일 분석
    const changeAnalysis = await this.analyzeFileChange(filePath)

    // 영향받는 문서 식별
    const affectedDocs = await this.identifyAffectedDocuments(filePath, changeAnalysis)
  ...
  private async updateDocumentIncremental(
    ...
    // 부분 재생성 (전체 재생성 대신)
    const partialUpdate = await this.generatePartialUpdate(document, impactScope, change)

    // 문서 일관성 검증
    const consistencyCheck = await this.validateDocumentConsistency(document, partialUpdate)
...
class DocumentationQualityAssessment {
  async assessDocumentationQuality(documentation: DocumentationSet): Promise<QualityAssessment> {
    const metrics = {
  ...
  // 문서 완성도 평가
  private async assessCompleteness(documentation: DocumentationSet): Promise<CompletenessScore> {
    const coverage = {
  ...
  async createQualityImprovementPlan(
    ...
    const prioritizedImprovements = this.prioritizeImprovements(assessment.actionItems, resources)

    const timeline = await this.createImprovementTimeline(prioritizedImprovements, resources)
