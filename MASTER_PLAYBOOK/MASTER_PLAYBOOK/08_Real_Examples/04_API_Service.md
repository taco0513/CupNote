...
export class PostService {
  ...
  async create(
    ...
    if (files && files.length > 0) {
      mediaUrls = await Promise.all(files.map(file => this.fileService.uploadImage(file, 'posts')))
    }
  ...
  async delete(postId: string, userId: string): Promise<void> {
    ...
    if (post.mediaUrls && post.mediaUrls.length > 0) {
      await Promise.all((post.mediaUrls as string[]).map(url => this.fileService.deleteFile(url)))
    }
  ...
  async getFeed(
    ...
    const transformedPosts = posts.map(post => this.transformPostToDto(post, post.likes.length > 0))
...
export class SearchService {
    ...
    try {
      const response = await this.elasticClient.search({
        ...
        body: {
          ...
          sort: [{ _score: { order: 'desc' } }, { followersCount: { order: 'desc' } }],
          from: offset,
          size: limit,
        },
      })
  ...
  async getTrendingTags(limit: number = 10): Promise<string[]> {
    try {
      ...
      return response.body.aggregations.trending_tags.buckets.map((bucket: any) => bucket.key)
    } catch (error) {
