...
class SprintPlanningManager {
  async planFirstSprint(
    ...
    // 5. 스프린트 백로그 구성
    const sprintBacklog = await this.createSprintBacklog(estimatedTasks, team.velocity, duration)
...
class MVPScopeDefiner {
  async defineMVPScope(project: Project): Promise<MVPScope> {
    ...
    const userFlows = [
      {
        name: '신규 사용자 온보딩',
        steps: ['홈페이지 방문', '회원가입 클릭', '정보 입력', '이메일 인증', '첫 로그인'],
        priority: 'Critical',
      },
      {
        name: '핵심 기능 사용',
    ...
    // 범위 외 항목
    const outOfScope = ['고급 분석 기능', '소셜 로그인', '모바일 앱', '다국어 지원', 'API 공개']
...
class UserStoryGenerator {
  generateUserStory(persona: UserPersona, need: string, value: string): UserStory {
    return {
      id: this.generateId(),
...
class PlanningPokerEstimator {
  ...
  async estimateTask(task: Task, team: TeamMember[]): Promise<TaskEstimation> {
    // 각 팀원의 예상
    const estimates = await Promise.all(team.map(member => this.getMemberEstimate(member, task)))
...
class BurndownChartGenerator {
  generateBurndownData(sprint: Sprint, dailyProgress: DailyProgress[]): BurndownChartData {
    const idealLine = this.calculateIdealLine(sprint.totalPoints, sprint.duration)
...
class DailyScrumTemplate {
  ...
  async recordDailyScrum(team: TeamMember[], date: Date): Promise<DailyScrumRecord> {
    const updates = await Promise.all(team.map(member => this.getMemberUpdate(member)))
...
class SprintRetrospective {
  async conductRetrospective(sprint: CompletedSprint): Promise<RetrospectiveResults> {
    // 정량적 분석
    const metrics = {
    ...
    return {
      ...
      feedback,
      improvements,
      nextSprintRecommendations: this.generateRecommendations(metrics, feedback, improvements),
    }
  }
}
```
