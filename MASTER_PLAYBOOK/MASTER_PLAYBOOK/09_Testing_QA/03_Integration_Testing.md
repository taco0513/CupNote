...
describe('UserRepository Integration Tests', () => {
  ...
  describe('create', () => {
    ...
    it('필수 필드 누락 시 오류를 발생시킨다', async () => {
      ...
      await expect(userRepository.create(incompleteData as any)).rejects.toThrow()
    })
  })
  ...
  describe('update', () => {
    ...
    it('존재하지 않는 사용자 업데이트 시 오류를 발생시킨다', async () => {
      await expect(userRepository.update(99999, { firstName: 'New Name' })).rejects.toThrow(
        'User not found'
      )
    })
  ...
  describe('복잡한 쿼리 테스트', () => {
    ...
    it('검색 기능이 올바르게 작동한다', async () => {
      ...
      expect(
        searchResults.every(
          user => user.firstName.includes('Active') || user.lastName.includes('Active')
        )
      ).toBe(true)
    })
  })
...
describe('PaymentService Integration Tests', () => {
  ...
  describe('processPayment', () => {
    ...
    it('결제 실패 시 적절한 오류를 반환한다', async () => {
      ...
      // Act & Assert
      await expect(paymentService.processPayment(validPaymentRequest)).rejects.toThrow(
        '유효하지 않은 카드 정보입니다.'
      )
    })
    ...
    it('다양한 결제 시나리오를 테스트한다', async () => {
      ...
      for (const scenario of scenarios) {
        ...
        // Act & Assert
        await expect(paymentService.processPayment(scenario.request)).rejects.toThrow(
  ...
  describe('refundPayment', () => {
    ...
    it('이미 환불된 결제에 대해 중복 환불 오류를 반환한다', async () => {
      ...
      // Act & Assert
      await expect(paymentService.refundPayment('payment_12345', 5000)).rejects.toThrow(
        '이미 환불된 결제입니다.'
      )
    })
  })
