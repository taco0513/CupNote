...
class BusinessRegistrationChecker {
  shouldRegister(): RegistrationDecision {
    const criteria = {
      ...
      // 아직 기다려도 OK
      canWait: ['무료 베타 테스트 중', '취미 프로젝트', '아직 수익 모델 불확실', '개인 학습 목적'],
    }
  ...
  getRequirements(): PrivacyRequirements {
    return {
      ...
      requiredPages: [
        ...
        {
          page: '서비스 이용약관',
          mustInclude: ['서비스 내용', '사용자 의무', '금지사항', '책임 제한', '분쟁 해결'],
...
class PaymentSystemComparison {
  compareOptions(): PaymentComparison[] {
    return [
      {
        provider: 'Stripe',
        pros: ['글로벌 지원 최고', '개발자 친화적 API', '다양한 결제 수단', '구독 결제 지원 우수'],
      ...
      {
        provider: '토스페이먼츠',
        pros: ['한국 카드사 직접 연동', '낮은 수수료', '간편한 연동', '한국어 지원'],
  ...
  getEssentials(): TaxEssentials {
    return {
      ...
      deductibleExpenses: [
        {
          category: '개발 비용',
          items: ['서버 호스팅', '도메인', 'API 사용료', '개발 도구 라이센스', '클라우드 서비스'],
        },
        {
          category: '마케팅',
    ...
    return [
      {
        ...
        redFlags: ['무제한 책임 조항', '모호한 범위 정의', '일방적 변경 권한', '과도한 위약금'],
      },
      {
        type: '프리랜서 계약서',
        essentialClauses: [
      ...
      {
        type: 'NDA (비밀유지계약)',
        essentialClauses: ['비밀 정보 정의', '사용 제한', '계약 기간', '위반 시 책임', '예외 사항'],
      },
    ]
  }
