...
class RealisticTimeEstimator {
  estimateTask(task: Task): TimeEstimate {
    ...
    const totalMultiplier = Object.values(realityMultipliers).reduce((acc, mult) => acc * mult, 1.0)
  ...
  handleDelays(delay: Delay): ResponsePlan {
    const strategies = {
      ...
      criticalDelay: {
        // 2개월 이상 지연
        action: '프로젝트 피벗',
        options: ['핵심 가치만 남기고 재설계', '시장 기회 재평가', '투자자/이해관계자와 재협의'],
      },
    }
  ...
  analyzeTeamNeeds(project: Project): TeamAnalysis {
    return {
      ...
      soloTeam: {
        ...
        timeline: '12개월',
        limitations: ['복잡한 커스터마이징 제한', '확장성 고려 후순위', '전문 영역 품질 타협'],
      },
    }
  }
...
class OutsourcingDecisionMaker {
  evaluateOptions(task: Task): OutsourcingAnalysis {
    ...
    return {
      ...
      options: {
        inhouse: {
          pros: ['완전한 통제권', '팀 역량 향상', '장기적 유지보수 용이', '비즈니스 도메인 이해'],
        ...
        outsourcing: {
          ...
          cons: ['커뮤니케이션 비용', '품질 통제 어려움', '의존성 발생', '지식 이전 부족'],
        ...
        hybrid: {
          ...
          strategy: ['아키텍처 설계: 내부', '반복 개발: 외주', '핵심 로직: 내부', 'UI 구현: 외주'],
  ...
  getFundingOptions(): FundingOptions {
    return {
      ...
      bootstrapping: {
        ...
        requirements: ['생활비 6-12개월분 확보', '최소 기능으로 시작', '빠른 수익 창출 모델'],
      },
      ...
      government_support: {
        ...
        success_rate: '10-30%',
        amounts: ['예비창업패키지: 최대 1억원', 'TIPS: 최대 5억원', '창업도약패키지: 최대 3억원'],
      },
  ...
  assessTechnicalDebt(): DebtAssessment {
    return {
      ...
      necessaryDebt: {
        ...
        time_constraints: {
          examples: ['임시 데이터 구조', '단순화된 권한 시스템', '수동 배포 프로세스'],
      ...
      dangerousDebt: {
        security_shortcuts: {
          examples: ['약한 인증 시스템', 'SQL 인젝션 취약점', '개인정보 평문 저장'],
          timeline: '즉시 해결',
          risk: '매우 높음',
        },
