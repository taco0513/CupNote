...
class AutoDocsGenerator {
  ...
  async startWatching() {
    ...
    const watchPaths = this.config.watch.paths.map(p => path.join(this.projectRoot, p))
  ...
  async performDocumentReview() {
    try {
      ...
      if (reviewResults && reviewResults.overallScore < 80) {
        console.log(
          chalk.yellow(`⚠️ 문서 품질 개선이 필요합니다 (점수: ${reviewResults.overallScore}/100)`)
        )
        ...
        console.log(
          chalk.green(`✅ 문서 품질이 양호합니다 (점수: ${reviewResults.overallScore}/100)`)
        )
      }
    } catch (error) {
      ...
      for (const file of lowQualityFiles.slice(0, 3)) {
        ...
        if (fs.existsSync(filePath)) {
          ...
          if (aiAnalysis && aiAnalysis.recommendations.length > 0) {
            console.log(
              chalk.cyan(`💡 ${file.path}: ${aiAnalysis.recommendations.length}개 AI 추천사항 생성`)
            )
          }
        }
      }
