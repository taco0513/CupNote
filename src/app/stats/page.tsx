...
const analyzeRecords = (records: CoffeeRecord[]) => {
  ...
  // 원산지별 분포
  const originData = records.reduce((acc, record) => {
  ...
  // 로스팅 레벨별 분포
  const roastData = records.reduce((acc, record) => {
  ...
  // 모드별 분포
  const modeData = records.reduce((acc, record) => {
  ...
  // 최고/최저 평점 커피
  const sortedByRating = [...records].sort((a, b) => b.rating - a.rating)
  const topRatedCoffee = sortedByRating[0]
  const lowestRatedCoffee = sortedByRating[sortedByRating.length - 1]

  // 가장 많이 방문한 카페/로스터리
  const roasteryData = records.reduce((acc, record) => {
    if (record.roastery) {
...
const BarChart = ({ 
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-coffee-100">
      <h3 className="text-lg font-semibold text-coffee-800 mb-4">{title}</h3>
...
const MonthlyChart = ({ 
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-coffee-100">
      <h3 className="text-lg font-semibold text-coffee-800 mb-4">월별 기록 추세</h3>
    ...
    return (
      <div className="min-h-screen bg-coffee-50 p-4">
        <div className="max-w-4xl mx-auto pt-4">
          ...
          <h1 className="text-3xl font-bold text-coffee-800 mb-8">☕ 커피 통계</h1>
  ...
  return (
    <div className="min-h-screen bg-coffee-50 p-4">
      <div className="max-w-6xl mx-auto pt-4">
        ...
        {/* 헤더 */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-coffee-800 mb-2">☕ 커피 통계</h1>
          <p className="text-coffee-600">나만의 커피 여정을 데이터로 살펴보세요</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <StatCard
            icon={Target}
            title="최고 평점"
            value={stats.topRatedCoffee.rating}
