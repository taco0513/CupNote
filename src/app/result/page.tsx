...
const getScoreGrade = (score: number) => {
  if (score >= 90) return { grade: 'S', color: 'from-yellow-400 to-orange-500', text: 'text-yellow-600' }
  if (score >= 80) return { grade: 'A', color: 'from-green-400 to-green-600', text: 'text-green-600' }
  if (score >= 70) return { grade: 'B', color: 'from-blue-400 to-blue-600', text: 'text-blue-600' }
  if (score >= 60) return { grade: 'C', color: 'from-purple-400 to-purple-600', text: 'text-purple-600' }
...
export default function ResultPage() {
  ...
  useEffect(() => {
    if (recordId) {
      const foundRecord = LocalStorage.getRecordById(recordId)
      if (foundRecord) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-coffee-50 to-coffee-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/" className="px-6 py-3 bg-coffee-600 text-white rounded-full hover:bg-coffee-700 transition-colors">
            홈으로 돌아가기
          </Link>
        </div>
      </div>
            ...
            <div className="text-center mb-8">
              ...
              <div className="relative mx-auto mb-6" style={{ width: '200px', height: '200px' }}>
                ...
                <div className="absolute inset-0 flex items-center justify-center flex-col">
                  <div className={`text-5xl font-bold bg-gradient-to-r ${scoreGrade.color} bg-clip-text text-transparent transition-all duration-1000 ${showAnimation ? 'scale-100' : 'scale-0'}`}>
                    {showAnimation ? matchScore : 0}
                  </div>
                  <div className={`text-3xl font-bold ${scoreGrade.text} transition-all duration-1000 delay-500 ${showAnimation ? 'opacity-100' : 'opacity-0'}`}>
                    {scoreGrade.grade}
                  </div>
                </div>
              </div>
            ...
            <div>
              <div className="flex items-center mb-3">
                <div className={`
                  ...
                  ${record.mode === 'cafe' ? 'bg-blue-100 text-blue-800' :
                    record.mode === 'homecafe' ? 'bg-green-100 text-green-800' :
