...
const saveResult = async () => {
  try {
    ...
    if (!isDemo) {
      ...
      notificationStore.showSuccess('테이스팅 결과가 저장되었습니다!', '💾 저장 완료')
    }
  } catch (error) {
    notificationStore.showError('저장 중 오류가 발생했습니다.', '❌ 저장 실패')
  }
}
```
...
<template>
  <div class="result-view">
    ...
    <section class="tasting-summary">
      ...
      <div class="summary-card">
        ...
        <div class="flavor-tags">
          <span v-for="flavor in selectedFlavors" :key="flavor" class="flavor-tag">
            {{ flavor }}
          </span>
        </div>
      </div>
      ...
      <div class="summary-card">
        ...
        <div class="sensory-tags">
          <span v-for="sensory in selectedSensory" :key="sensory" class="sensory-tag">
            {{ sensory }}
          </span>
        </div>
      </div>
    ...
    <footer class="result-actions">
      <button v-if="!isDemo" @click="saveResult" class="btn-primary">💾 결과 저장</button>

      <button @click="startNewTasting" class="btn-secondary">🔄 새로운 테이스팅</button>

      <button v-if="isDemo" @click="goToSignup" class="btn-cta">🚀 가입하고 기록 저장하기</button>

      <button v-else @click="goToStats" class="btn-outline">📊 통계 보기</button>
    </footer>
  </div>
</template>
```
