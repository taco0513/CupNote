...
export const useDemoStore = defineStore('demo', () => {
  ...
  const demoData = ref({
    ...
    personal_comment: {
      ...
      free_text:
        '블루베리와 레몬의 밝은 산미가 인상적이고, 재스민 향이 우아하게 느껴집니다. 아침에 마시기 정말 좋은 커피네요!',
    },
    roaster_notes:
      '에티오피아 구지 지역의 헤이룸 품종으로, 블루베리와 레몬의 밝은 산미, 재스민의 플로럴 향이 특징입니다. 라이트 로스팅으로 원두 본연의 향미를 살렸습니다.',
  })
  ...
  const getStepGuidance = (stepName: string) => {
    const guidance = {
      ...
      'coffee-info': {
        title: '☕ 커피 정보를 확인하세요',
        message:
          '데모용 커피 정보가 미리 입력되어 있습니다. 실제 사용시에는 직접 입력하실 수 있어요!',
      },
      'flavor-selection': {
        title: '🍓 향미를 선택해보세요',
