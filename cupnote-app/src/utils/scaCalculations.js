...
export function calculateScaComplianceScore(brewingData) {
  const { coffeeAmount, waterAmount, waterTemp, waterTds, waterPh } = brewingData
...
export function predictQualityScore(params) {
  const { tds, brewVolume, coffeeAmount, scaComplianceScore, extractionYield } = params
  ...
  if (tds && brewVolume && coffeeAmount) {
    const extractionYieldValue =
      extractionYield || calculateExtractionYield(tds, brewVolume, coffeeAmount)
