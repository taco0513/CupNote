...
function calculateFlavorMatch(userFlavors: string[], roasterFlavors: string[]): number {
  ...
  const matches = userFlavors.filter((flavor) => roasterFlavors.includes(flavor)).length
...
describe('matchCalculator', () => {
  ...
  describe('calculateFlavorMatch', () => {
    it('모든 향미가 일치하면 100점을 반환한다', () => {
      const score = calculateFlavorMatch(['chocolate', 'caramel'], ['chocolate', 'caramel'])
      expect(score).toBe(100)
    })

    it('일부 향미만 일치하면 부분 점수를 반환한다', () => {
      const score = calculateFlavorMatch(['chocolate', 'fruity', 'nutty'], ['chocolate', 'caramel'])
      expect(score).toBeGreaterThan(0)
      expect(score).toBeLessThan(100)
    })

    it('일치하는 향미가 없으면 0점을 반환한다', () => {
      const score = calculateFlavorMatch(['fruity', 'floral'], ['chocolate', 'caramel'])
      expect(score).toBe(0)
    })
